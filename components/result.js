...
const Result = () => {
  ...
  const [config, setConfig] = useControls(() => ({
    types: { value: true, hint: 'Add Typescript definitions' },
    ...
    keepnames: { value: true, label: 'keep names', hint: 'Keep original names' },
    keepgroups: { value: false, label: 'keep groups', hint: 'Keep (empty) groups' },
    meta: { value: false, hint: 'Include metadata (as userData)' },
    precision: { value: 3, min: 1, max: 8, step: 1, hint: 'Number of fractional digits (default: 3)' },
    pathPrefix: {
      label: 'path prefix',
      value: 'assets/farmbot/3D/models',
  ...
  const preview = useControls(
    'preview',
    {
      autoRotate: false,
      contactShadow: true,
      intensity: { value: 0, min: 0, max: 2, step: 0.1, label: 'light intensity' },
      preset: {
        value: 'upfront',
